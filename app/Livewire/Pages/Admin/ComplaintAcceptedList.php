...
class ComplaintAcceptedList extends Component
{
    private function getDataForDataTable()
    {
        ...
        $accepted_complaints = AcceptComplaint::with(['complaint:id,title,report_category,created_at'])
            ->whereNull('doned_at') // Only get complaints that are not completed
        ...
        $formattedData = $accepted_complaints->map(function ($accepted_complaint) {
            ...
            $btnDetails = '<a href="' . route('accepted.show', $accepted_complaint) . '">
    ...
    public function render()
    {
        ...
        return view('livewire.pages.admin.complaint-accepted-list', compact('data'));
    }
}
